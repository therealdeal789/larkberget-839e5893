...
const InvestorsOverview = () => {
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      <section className="bg-gradient-to-br from-slate-800 to-slate-900 text-white pt-20 pb-16">
        <div className="container mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-12">
            {keyFacts.map((fact, index) => (
              <div 
                key={index}
                className="bg-slate-700/50 backdrop-blur-sm rounded-lg p-6 text-center hover:bg-slate-700/70 transition-all duration-300"
              >
                <fact.icon className="w-8 h-8 text-trust-400 mx-auto mb-4" />
                <h3 className="font-semibold text-lg mb-2">{fact.title}</h3>
                <p className="text-slate-300 text-sm">{fact.description}</p>
              </div>
            ))}
          </div>
          ...
          <div className="bg-yellow-500/20 border border-yellow-500/30 rounded-lg p-4 mb-8">
            <p className="text-center text-sm">
              ...
              <button className="underline hover:no-underline ml-1">Läs mer här</button>
            </p>
          </div>
